...
-(id) initWithStyle:(UITableViewStyle)style {
    if (self = [super initWithStyle:style]) {
        ...
        projectTitles = [[NSArray alloc] init];
    }
    return self;
}
...
-(void) setProjectTitles:(NSArray*) newProjectTitles {
    [newProjectTitles retain];
    [projectTitles release];
    projectTitles = newProjectTitles;
    NSLog(@"rVC set its projectTitles to be %@", projectTitles);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [(WondewAppDelegate *) [[UIApplication sharedApplication] delegate] projectTapped:indexPath.row];
