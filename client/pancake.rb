...
class Pancake
  ...
  def find_lines(args)#regex, offset=0, limit=0, starttime=0, endtime=0) 
    args[:regex] ||= ''
    args[:regex] = args[:regex].inspect[1..-2] if args[:regex].is_a? Regexp
    Pancake.get("/#{@username}/#{@application_name}", :query=>{:secret=>@secret}.update(args)).parsed_response#, :regex=>regex, :offset=>offset, :limit=>limit, :starttime=>starttime, :endtime=>endtime}).parsed_response
  end
  ...
  def replace_lines(lines, offset=0, limit=0)
    lines = JSON.dump(lines)
    puts "dumped ok!"
    Pancake.put("/#{@username}/#{@application_name}", :query=>{:secret=>@secret, :lines=>lines, :offset=>offset, :limit=>limit}).parsed_response
  end
